...
@section('content-header')
    <h1>
        {{ trans('voucher::userregistrations.title.userregistrations') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('voucher::userregistrations.title.userregistrations') }}</li>
    </ol>
@stop
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('voucher::userregistrations.title.create userregistration') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.voucher.userregistration.create') ?>" }
                ]
            });
        });
    </script>
